...
import (
	...
	"github.com/sgostarter/libeasygo/crypt/ssr/saltseed"
)
...
func (c *streamRW) initReader() error {
	...
	if saltseed.CheckSalt(salt) {
		return ErrRepeatedSalt
	}

	c.r = newReader(c.rw, aead)

	return nil
}

func (c *streamRW) Read(b []byte) (int, error) {
	if c.r == nil {
...
func (c *streamRW) WriteTo(w io.Writer) (int64, error) {
	if c.r == nil {
		if err := c.initReader(); err != nil {
			return 0, err
		}
...
func (c *streamRW) initWriter() error {
	salt := make([]byte, c.SaltSize())
	...
	_, err = c.rw.Write(salt)
	if err != nil {
		return err
	}

	saltseed.AddSalt(salt)

	c.w = newWriter(c.rw, aead)

	return nil
}

func (c *streamRW) Write(b []byte) (int, error) {
	if c.w == nil {
...
func (c *streamRW) ReadFrom(r io.Reader) (int64, error) {
	if c.w == nil {
		if err := c.initWriter(); err != nil {
			return 0, err
		}
...
// NewRW wraps a stream-oriented net.Conn with cipher.
func NewRW(rw io.ReadWriter, ciph Cipher) io.ReadWriter { return &streamRW{rw: rw, Cipher: ciph} }
