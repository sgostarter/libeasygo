...
// A shared instance used for checking saltseed repeat
	...
	initSaltfilterOnce.Do(func() {
		...
		for _, opt := range []struct {
			...
			if env != "" {
				...
				if err != nil {
					panic(fmt.Sprintf("Invalid environment `%s` setting in saltfilter: %s", envKey, env))
				}
...
// TestSalt returns true if saltseed is repeated
func TestSalt(b []byte) bool {
	return getSaltFilterSingleton().Test(b)
}

// AddSalt saltseed to filter
func AddSalt(b []byte) {
